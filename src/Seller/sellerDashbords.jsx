...
import {
  LayoutDashboard, Package, MessageSquare, GitCommit, BarChart2, ShieldCheck,
  X, Users, Eye, Menu, PlusCircle, UploadCloud, TrendingUp, LogOut, Loader2, MapPin
} from "lucide-react";
...
const SellerDashboard = () => {
 ...
 // Frontend API Call simplified
const fetchMyProducts = async () => {
  setFetchingProducts(true);
  try {
    // Bas itna kafi hai, backend cookie se ID nikal lega
 ...
 const handleLogout = async () => {
  ...
  const handleImageChange = (e) => { if (e.target.files) setProductImage(e.target.files[0]); };
  ...
  const stats = [
    ...
    { label: "Active Inquiries", value: "2", icon: Users, color: "text-green-600", bg: "bg-green-100" },
    ...
    <div className="flex h-[calc(100vh-64px)] bg-slate-50 overflow-hidden font-sans relative">
      ...
      <div className="flex-1 overflow-y-auto w-full">
        <div className="bg-white px-8 py-4 border-b border-slate-200 flex justify-between items-center sticky top-0 z-10">
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden p-2 rounded-lg bg-slate-100 text-slate-600"><Menu size={20} /></button>
        </div>
          ...
          {activeTab === "dashboard" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="bg-blue-600 rounded-3xl p-8 text-white flex flex-col md:flex-row justify-between items-start md:items-center shadow-lg shadow-blue-600/20 gap-4">
                ...
                <button onClick={() => setIsAddModalOpen(true)} className="bg-white text-blue-600 font-bold px-6 py-3.5 rounded-xl flex items-center gap-2 hover:bg-slate-50 transition-colors shadow-sm">
          ...
          {activeTab === "products" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setIsAddModalOpen(true)} className="bg-blue-600 text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-700 text-sm shadow-md">
              ...
              {fetchingProducts ? (
                ...
                <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center flex flex-col items-center">
                  ...
                  <p className="text-sm text-slate-500 mb-4">You haven't added any products to your global exchange yet.</p>
                  <button onClick={() => setIsAddModalOpen(true)} className="bg-blue-50 text-blue-600 font-bold px-5 py-2.5 rounded-lg flex items-center gap-2 hover:bg-blue-100 transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {myProducts.map((product) => (
                    <div key={product._id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-lg transition-all group">
                      <div className="h-48 overflow-hidden bg-slate-100 relative">
                        {product.productImage ? (
                          <img src={product.productImage} alt={product.productName} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        ) : (
                          <div className="flex items-center justify-center h-full"><Package size={40} className="text-slate-300"/></div>
                        )}
                        <span className={`absolute top-3 right-3 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider rounded-full ${product.verifiedExporter === 'Verified' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                      ...
                      <div className="p-5">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md mb-2 inline-block">{product.category}</span>
                            <h3 className="text-lg font-black text-slate-800 line-clamp-1">{product.productName}</h3>
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-50 rounded-xl p-3 grid grid-cols-2 gap-2 text-sm">
                          <div>
                            <p className="text-xs text-slate-500 font-bold uppercase">Min. Order</p>
                            <p className="font-semibold text-slate-700">{product.moq} {product.moqUnit}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 font-bold uppercase">Company</p>
                            <p className="font-semibold text-slate-700 truncate">{product.companyName || "N/A"}</p>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <button onClick={() => setIsAddModalOpen(false)} className="text-slate-400 hover:text-slate-600 hover:bg-slate-200 p-2 rounded-full transition-all">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-8 overflow-y-auto">
              <form className="space-y-6">
                <div className="border-2 border-dashed border-slate-200 rounded-2xl p-8 flex flex-col items-center justify-center cursor-pointer hover:bg-blue-50 hover:border-blue-400 transition-all relative group">
                  <UploadCloud size={40} className="text-blue-500 mb-3 group-hover:scale-110 transition-transform" />
                  <p className="text-sm font-semibold text-slate-700">{productImage ? productImage.name : "Upload Product Image"}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {[
                    ...
                    <div key={field.name} className="flex flex-col gap-1.5">
                      ...
                      {field.type === "select" ? (
                        <select name={field.name} value={productData[field.name]} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all text-sm bg-white">
                        ...
                        <input type={field.type} name={field.name} value={productData[field.name]} onChange={handleInputChange} placeholder={field.placeholder} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all text-sm" />
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea name="description" value={productData.description} onChange={handleInputChange} placeholder="Describe your product quality, origins, or unique selling points..." rows="3" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all text-sm resize-none"></textarea>
                </div>
              </form>
            </div>

            <div className="px-8 py-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              <button onClick={() => setIsAddModalOpen(false)} className="px-6 py-3 rounded-xl font-bold text-slate-600 hover:bg-slate-200 transition-all text-sm">Cancel</button>
              <button onClick={handleSubmitProduct} disabled={loading} className="px-6 py-3 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-600/20 transition-all text-sm flex items-center gap-2">
